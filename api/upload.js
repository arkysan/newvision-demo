...
module.exports = async (req, res) => {
  ...
  if (!ALLOWED_TYPES.has(contentType)) {
    return send(res, 400, { ok: false, error: 'Image type not allowed. Use JPEG, PNG, WebP, or GIF.' });
  }
  ...
  const safe = String(raw).replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 80) || 'photo.jpg';
  const pathname = `vehicles/${Date.now()}_${safe}`;
